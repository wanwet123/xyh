...
function getCityBuy(val){
	...
	newStr.push("<li onclick=\"getProvinceBuy()\" style=\"background-color:#C4C4C4;\">"+province[val].NAME+"</li>");
...
function getAreaBuy(val,val1){
	...
	var newStr=new Array();
	newStr.push("<div class=\"dqld_div\"><ul>");
	newStr.push("<li onclick=\"getProvinceBuy()\" style=\"background-color:#C4C4C4;\">"+province[val1].NAME+"</li>");
	newStr.push("<li onclick=\"getCityBuy("+val1+")\" style=\"background-color:#E0E0E0;padding-left:10px;\">"+city[val].NAME+"</li>");
	...
	if(asize==0){
		...
		var $sheng = $('#sheng'),
		$shi = $('#shi');
		$sheng.val(province[val1].NAME);
		$shi.val(city[val].NAME);
...
function getallArea(val,val1,val2){
	...
	var allarea=province[val].NAME+' '+city[val1].NAME+' '+area[val2].NAME;  //最后获得的省市区之间加个空格
	...
	var $sheng = $('#sheng'),
	    $shi = $('#shi'),
	    $qu = $('#qu');
    $sheng.val(province[val].NAME);
    $shi.val(city[val1].NAME);
    $qu.val(area[val2].NAME);
}
/*地区联动*/